...
class Custom_Dataset():
    ...
    def prepare_data(self, input_size, transform_method):
        ...
        try:
            train_dataset = datasets.ImageFolder(train_path,train=True, transform=self.data_transforms(input_size, transform_method))
        except Exception as ex:
            print("Exception caught - NO dataset found  in the specified folder. Please ensure you kept the train in data folder")
            print("Exception:{}".format(ex))

        try:
            validation_dataset = datasets.ImageFolder(validation_path,train=False, transform=self.data_transforms(input_size, transform_method))
        except Exception as ex:
            print("Exception caught - NO dataset found  in the specified folder. Please ensure you kept the validation in data folder")
            print("Exception:{}".format(ex))
        

        try:
        # For code validation purposes test set and val set are same
            test_dataset = datasets.ImageFolder(test_path,train=False, transform=self.data_transforms(input_size,transform_method))
        except Exception as ex:
            print("Exception caught - NO dataset found  in the specified folder. Please ensure you kept the test set in data folder")
            print("Exception:{}".format(ex))
