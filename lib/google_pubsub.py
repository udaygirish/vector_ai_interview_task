...
class GcpPubSub():
    ...
    def create_publisher(self, topic_name):
        ...
        self.topic_name = 'projects/{project_id}/topics/{topic}'.format(project_id=self.project_id,topic=topic_name)
        self.publisher.create_topic(name = self.topic_name)

    def create_subscriber(self, subscriber_name):
        self.subscriber = pubsub_v1.SubscriberClient()
        self.subscription_name = 'projects/{project_id}/subscriptions/{sub}'.format(project_id=self.project_id,sub=subscriber_name)
    ...
    def publish_message(self, message):
        ...
        streaming_put = self.publisher.publish(self.topic_name, message, origin='sample_app')
    ...
    def receive_message(self, message, timeout=5.0):
        streaming_pull = self.subscriber.subscribe(self.subscription_name, callback= self.callback)
